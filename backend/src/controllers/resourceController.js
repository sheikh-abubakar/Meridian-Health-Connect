import { AuditLog } from "../models/AuditLog.js";
import { Location } from "../models/Location.js";
import { Resource } from "../models/Resource.js";
import { ApiError } from "../utils/ApiError.js";
import { asyncHandler } from "../utils/asyncHandler.js";

const scope = (req) => ({ tenantId: req.tenantId, locationId: req.locationId });

export const listResources = asyncHandler(async (req, res) => {
  const resources = await Resource.find({ ...scope(req), isActive: { $ne: false } }).sort({ type: 1, name: 1 }).lean();
  res.json({ success: true, data: { resources } });
});

export const createResource = asyncHandler(async (req, res) => {
  const name = String(req.body.name || "").trim(); const type = String(req.body.type || "");
  if (!name) throw new ApiError(400, "Resource name is required");
  if (!["room", "equipment", "telehealth_link"].includes(type)) throw new ApiError(400, "Choose a valid resource type");
  const resource = await Resource.create({ ...scope(req), name, type });
  await AuditLog.create({ ...scope(req), actorUserId: req.user._id, action: "resource_created", targetType: "Resource", targetId: resource._id });
  res.status(201).json({ success: true, data: { resource } });
});

export const updateSchedulingSettings = asyncHandler(async (req, res) => {
  const max = Number(req.body.maxOverbookSlotsPerDoctorPerDay);
  if (!Number.isInteger(max) || max < 0 || max > 20) throw new ApiError(400, "Overbook limit must be a whole number from 0 to 20");
  const inputRules = Array.isArray(req.body.reminderRules) ? req.body.reminderRules : [];
  const reminderRules = inputRules.map((rule) => ({ channel: String(rule.channel), offsetHours: Number(rule.offsetHours) }));
  if (reminderRules.some((rule) => rule.channel !== "sms" || !Number.isFinite(rule.offsetHours) || rule.offsetHours < 0 || rule.offsetHours > 8760)) throw new ApiError(400, "Only SMS reminders are currently supported; select an offset from 0 to 8760 hours");
  const tier2 = Number(req.body.escalateToCreatorAfterDays);
  const tier3 = Number(req.body.flagToAdminAfterDays);
  if (!Number.isInteger(tier2) || tier2 < 1 || tier2 > 365) throw new ApiError(400, "Creator escalation threshold must be a whole number from 1 to 365 days");
  if (!Number.isInteger(tier3) || tier3 < tier2 || tier3 > 365) throw new ApiError(400, "Admin threshold must be a whole number at least as large as the creator threshold");
  const messageExpected = Number(req.body.patientMessageExpectedResponseHours ?? 24); const messageAdmin = Number(req.body.patientMessageFlagToAdminAfterHours ?? 48);
  if (!Number.isInteger(messageExpected) || messageExpected < 1 || messageExpected > 720 || !Number.isInteger(messageAdmin) || messageAdmin < messageExpected || messageAdmin > 720) throw new ApiError(400, "Message response thresholds must be valid whole hours, with Admin threshold after expected response time");
  const monitoringReview = Number(req.body.monitoringDoctorReviewAfterDays ?? 5); const monitoringAdherence = Number(req.body.monitoringAdherenceAfterDays ?? 5);
  if (!Number.isInteger(monitoringReview) || monitoringReview < 1 || monitoringReview > 365 || !Number.isInteger(monitoringAdherence) || monitoringAdherence < 1 || monitoringAdherence > 365) throw new ApiError(400, "Monitoring thresholds must be valid whole days from 1 to 365");
  const blackouts = Array.isArray(req.body.selfSchedulingBlackouts) ? req.body.selfSchedulingBlackouts.map((item) => ({ startDate: String(item.startDate || ""), endDate: String(item.endDate || "") })) : [];
  if (blackouts.some((item) => !/^\d{4}-\d{2}-\d{2}$/.test(item.startDate) || !/^\d{4}-\d{2}-\d{2}$/.test(item.endDate) || item.endDate < item.startDate)) throw new ApiError(400, "Each blackout window needs valid start and end dates");
  const showCarePlans = Boolean(req.body.showCarePlansInPatientPortal);
  const location = await Location.findOneAndUpdate({ _id: req.locationId, tenantId: req.tenantId }, { $set: { "schedulingSettings.maxOverbookSlotsPerDoctorPerDay": max, "schedulingSettings.reminderRules": reminderRules, "schedulingSettings.escalateToCreatorAfterDays": tier2, "schedulingSettings.flagToAdminAfterDays": tier3, "schedulingSettings.selfSchedulingBlackouts": blackouts, "schedulingSettings.patientMessageExpectedResponseHours": messageExpected, "schedulingSettings.patientMessageFlagToAdminAfterHours": messageAdmin, "schedulingSettings.monitoringDoctorReviewAfterDays": monitoringReview, "schedulingSettings.monitoringAdherenceAfterDays": monitoringAdherence, "schedulingSettings.showCarePlansInPatientPortal": showCarePlans } }, { new: true, runValidators: true }).lean();
  await AuditLog.create({ ...scope(req), actorUserId: req.user._id, action: "scheduling_settings_updated", targetType: "Location", targetId: location._id });
  res.json({ success: true, data: { settings: location.schedulingSettings } });
});
