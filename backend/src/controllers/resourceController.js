...
export const updateSchedulingSettings = asyncHandler(async (req, res) => {
  ...
  if (reminderRules.some((rule) => !["sms", "email", "voice"].includes(rule.channel) || !Number.isFinite(rule.offsetHours) || rule.offsetHours < 0 || rule.offsetHours > 8760)) throw new ApiError(400, "Reminder rules require a valid channel and offset from 0 to 8760 hours");
