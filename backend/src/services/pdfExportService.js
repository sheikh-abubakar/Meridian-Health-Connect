...
function createDocument(res, filename) {
  const doc = new PDFDocument({
    ...
    info: {
      Title: filename,
      Author: "Meridian Health Connect",
      Subject: "Patient-authorized health record export",
    },
  });
  res.status(200);
  res.set({
...
function letterhead(
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(25)
    .text("+", 63, 39, { width: 22, align: "center", lineBreak: false });
  doc
    .fillColor("#ffffff")
    .font("Helvetica-Bold")
    .fontSize(20)
    .text(tenant.name, 116, 31, { width: 431, lineBreak: false });
  doc
    .fillColor("#99f6e4")
    .font("Helvetica-Bold")
    .fontSize(10)
    .text(location.name, 116, 59, { width: 431, lineBreak: false });
  doc
    .fillColor("#cbd5e1")
    .font("Helvetica")
    .fontSize(9)
    .text(clean(location.address, "Address not recorded"), 116, 76, {
      width: 431,
      lineBreak: false,
    });
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(19)
    .text(documentTitle, left, 149);
  if (attendingPhysician) {
    doc
      .roundedRect(left, 181, contentWidth, 42, 6)
      .fillAndStroke("#f0fdfa", "#99f6e4");
    doc
      .fillColor(palette.muted)
      .font("Helvetica-Bold")
      .fontSize(7.5)
      .text("ATTENDING PHYSICIAN", left + 14, 191, {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(13)
      .text(clean(attendingPhysician), left + 168, 187, {
        width: contentWidth - 182,
        lineBreak: false,
        ellipsis: true,
      });
    doc.y = 242;
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(9)
      .text("Meridian Health Connect | Patient record export", left, 176);
    doc.y = 202;
  }
}
...
function sectionHeading(doc, title, subtitle = "") {
  ...
  if (subtitle) {
    doc
      ...
      .font("Helvetica")
      .fontSize(8.5)
      .text(subtitle, left, doc.y, { width: contentWidth });
    doc.moveDown(0.5);
  }
}
...
function metadataGrid(doc, items) {
  ...
  for (let index = 0; index < items.length; index += 2) {
    ...
    row.forEach(([label, value], column) => {
      ...
      doc
        .roundedRect(x, y, columnWidth, 34, 5)
        .fillAndStroke(palette.soft, "#e2e8f0");
      doc
        .fillColor(palette.muted)
        .font("Helvetica-Bold")
        .fontSize(7)
...
function clinicalBox(doc, label, value) {
  ...
  const y = doc.y;
  doc
    .roundedRect(left, y, contentWidth, height, 6)
    .fillAndStroke("#ffffff", "#dbe4eb");
  doc.rect(left, y, 5, height).fill(palette.teal);
  doc
    .fillColor(palette.teal)
    .font("Helvetica-Bold")
    .fontSize(8)
    .text(label.toUpperCase(), left + 18, y + 12, { characterSpacing: 0.5 });
  doc
    .fillColor(palette.text)
    .font("Helvetica")
    .fontSize(9.5)
    .text(text, left + 18, y + 29, { width: contentWidth - 32, lineGap: 2 });
  doc.y = y + height + 8;
}
...
function aiSummaryBox(doc, aiSummary) {
  ...
  const y = doc.y;
  doc
    .roundedRect(left, y, contentWidth, height, 7)
    .fillAndStroke(palette.violetSoft, "#c4b5fd");
  doc.circle(left + 22, y + 22, 10).fill(palette.violet);
  doc
    .fillColor("#ffffff")
    .font("Helvetica-Bold")
    .fontSize(10)
    .text("AI", left + 14, y + 18, {
      width: 16,
      align: "center",
      lineBreak: false,
    });
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(10)
    .text("CLINICAL SUMMARY (AI-ASSISTED)", left + 40, y + 16, {
      lineBreak: false,
    });
  doc
    .fillColor(palette.text)
    .font("Helvetica")
    .fontSize(9.5)
    .text(summary, left + 16, y + 43, { width: contentWidth - 32, lineGap: 2 });
  doc
    .fillColor(palette.muted)
    .font("Helvetica")
    .fontSize(7.5)
    .text(
      `Provider: Groq  |  Model: ${clean(aiSummary.model)}  |  Clinician accepted: ${dateTime(aiSummary.acceptedAt)}`,
      left + 16,
      y + height - 19,
      { width: contentWidth - 32, lineBreak: false, ellipsis: true },
    );
  doc.y = y + height + 10;
}
...
function amendmentsTimeline(doc, amendments) {
  if (!amendments.length) {
    doc
      ...
      .fontSize(9)
      .text("No amendments recorded.", left, doc.y);
    doc.moveDown(0.7);
    return;
  }
  for (const amendment of amendments) {
    ...
    const height = Math.max(
      58,
      doc.heightOfString(body, { width: contentWidth - 48, lineGap: 2 }) + 36,
    );
    ensureSpace(doc, height + 8);
    const y = doc.y;
    doc
      .moveTo(left + 10, y)
      .lineTo(left + 10, y + height)
      .strokeColor("#99f6e4")
      .lineWidth(2)
      .stroke();
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(9)
      .text(body, left + 28, y, { width: contentWidth - 40, lineGap: 2 });
    doc
      .fillColor(palette.muted)
      .font("Helvetica")
      .fontSize(7.5)
      .text(
        `${amendment.actor?.name || "Clinician"} | ${dateTime(amendment.timestamp)}`,
        left + 28,
        y + height - 18,
        { lineBreak: false },
      );
...
function carePlans(doc, plans, tasksByPlan) {
  for (const plan of plans) {
    ...
    const y = doc.y;
    doc
      .roundedRect(left, y, contentWidth, 76, 6)
      .fillAndStroke("#f0fdfa", "#99f6e4");
    doc
      .fillColor(palette.teal)
      .font("Helvetica-Bold")
      .fontSize(10)
      .text(plan.goal, left + 14, y + 12, { width: contentWidth - 28 });
    doc
      .fillColor(palette.muted)
      .font("Helvetica-Bold")
      .fontSize(7.5)
      .text("TARGET", left + 14, y + 35, { lineBreak: false });
    doc
      .fillColor(palette.text)
      .font("Helvetica")
      .fontSize(8.5)
      .text(clean(plan.targetMeasure), left + 14, y + 48, {
        width: 280,
        lineBreak: false,
        ellipsis: true,
      });
    doc
      ...
      .font("Helvetica-Bold")
      .fontSize(7.5)
      .text("REVIEW CADENCE", left + 330, y + 35, { lineBreak: false });
    doc
      .fillColor(palette.text)
      .font("Helvetica")
      .fontSize(8.5)
      .text(clean(plan.reviewCadence), left + 330, y + 48, {
        width: 150,
        lineBreak: false,
        ellipsis: true,
      });
    ...
    if (!tasks.length) {
      doc
        ...
        .font("Helvetica-Oblique")
        .fontSize(8.5)
        .text("No linked tasks.", left + 14, doc.y);
      doc.moveDown(0.6);
    }
    for (const task of tasks) {
      ...
      const height = Math.max(
        42,
        doc.heightOfString(body, { width: contentWidth - 112, lineGap: 2 }) +
          18,
      );
      ensureSpace(doc, height + 6);
      const taskY = doc.y;
      doc
        .roundedRect(left + 14, taskY, contentWidth - 14, height, 5)
        .fillAndStroke("#ffffff", "#e2e8f0");
      doc
        .fillColor(palette.text)
        .font("Helvetica")
        .fontSize(8.5)
        .text(body, left + 26, taskY + 10, {
          width: contentWidth - 126,
          lineGap: 2,
        });
      const statusColor =
        task.status === "completed" ? palette.teal : "#b45309";
      doc
        ...
        .text(
          ...
          left + contentWidth - 80,
          taskY + 11,
          { width: 66, align: "right", lineBreak: false },
        );
...
function visitTable(doc, appointments) {
  ...
  const drawHeader = () => {
    ...
    for (const column of columns)
      doc
        ...
        .font("Helvetica-Bold")
        .fontSize(7.5)
        .text(column.label, column.x + 8, y + 10, {
  ...
  appointments.forEach((appointment, index) => {
    ...
    const values = [
      ...
      clean(appointment.visitType),
      appointment.doctorId?.name || "Unavailable",
      String(appointment.status).replace("_", " "),
    ];
    values.forEach((value, columnIndex) =>
      doc
        .fillColor(columnIndex === 3 ? palette.teal : palette.text)
        .font(columnIndex === 3 ? "Helvetica-Bold" : "Helvetica")
        .fontSize(8)
        .text(value, columns[columnIndex].x + 8, y + 12, {
          width: columns[columnIndex].width - 16,
          lineBreak: false,
          ellipsis: true,
        }),
    );
  ...
  if (!appointments.length) {
    doc
      ...
      .font("Helvetica-Oblique")
      .fontSize(9)
      .text("No appointments recorded.", left + 10, doc.y + 14);
    doc.y += 42;
  }
}

function footer(doc, exportedBy, notice = "", actorLabel = "Exported by") {
  ...
  for (let index = range.start; index < range.start + range.count; index += 1) {
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(7.5)
      .text(`Generated ${generated}`, left, doc.page.height - 39, {
        width: 180,
        lineBreak: false,
      });
    doc.text(
      `${actorLabel} ${clean(exportedBy, "Authorized staff")} - Meridian Health Connect`,
      left + 150,
      doc.page.height - 39,
      { width: 270, align: "center", lineBreak: false, ellipsis: true },
    );
    doc.text(
      `Page ${index + 1} of ${range.count}`,
      left + 420,
      doc.page.height - 39,
      { width: 79, align: "right", lineBreak: false },
    );
...
export function renderEncounterPdf(
  res,
  { tenant, location, encounter, linkedCarePlans, tasksByPlan, exportedBy },
) {
  ...
  sectionHeading(
    doc,
    "Clinical Notes",
    "Clinician-authored documentation reproduced as recorded.",
  );
...
export function renderVisitHistoryPdf(
  res,
  {
    tenant,
    location,
  ...
  metadataGrid(doc, [
    ...
    ...(requestedByPatient ? [["Requested by", "Patient Portal"]] : []),
  ]);
  sectionHeading(
    doc,
    "Complete Visit History",
    "Administrative scheduling history; clinical detail is intentionally excluded.",
  );
  visitTable(doc, appointments);
  footer(
    doc,
    exportedBy,
    "Administrative export only - clinical notes, diagnoses, care-plan details, and task outcomes are excluded.",
    requestedByPatient ? "Requested by" : "Exported by",
  );
  doc.end();
}
