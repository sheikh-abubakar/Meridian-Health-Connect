...
export function MyPatientsPage() {
  const realtimeRevision = useRealtimeRevision(["encounter:finalized", "encounter:amended"]);
  ...
  useEffect(() => { apiRequest(`/${tenantSlug}/${locationSlug}/my-patients`, { headers }).then((data) => setPatients(data.patients)).catch((requestError) => setError(requestError.message)); }, [headers, locationSlug, realtimeRevision, tenantSlug]);
