...
export function StaffPage() {
  ...
  useEffect(() => {
    ...
    async function loadStaff() {
      try {
        const data = await apiRequest(`/${tenantSlug}/users`, { headers: { Authorization: `Bearer ${session.accessToken}` } });
  ...
  if (session.user.role !== "admin") return <Navigate to={`/${tenantSlug}/dashboard`} replace />;
  ...
  async function handleSubmit(event) {
    ...
    setSubmitting(true);
    try {
      const data = await apiRequest(`/${tenantSlug}/users`, {
        method: "POST",
  ...
  return <>
    ...
    <Card className="mt-8 bg-white shadow-none"><CardContent className="p-0">{loading ? <div className="space-y-3 p-6"><div className="h-4 w-48 animate-pulse rounded bg-slate-200" /><div className="h-12 animate-pulse rounded bg-slate-100" /><div className="h-12 animate-pulse rounded bg-slate-100" /></div> : loadError ? <div className="p-8 text-center text-sm text-red-700">{loadError}</div> : staff.length === 0 ? <div className="grid place-items-center px-6 py-16 text-center"><div className="grid size-11 place-items-center rounded-md bg-slate-100 text-slate-500"><Users className="size-5" /></div><p className="mt-4 font-medium">No staff records yet</p><p className="mt-1 text-sm text-muted-foreground">Add the first member of this clinic team.</p></div> : <Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Email</TableHead><TableHead>Role</TableHead></TableRow></TableHeader><TableBody>{staff.map((member) => <TableRow key={member.id}><TableCell className="font-medium">{member.name}</TableCell><TableCell className="text-muted-foreground">{member.email}</TableCell><TableCell><span className="inline-flex rounded-md bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">{roleLabels[member.role]}</span></TableCell></TableRow>)}</TableBody></Table>}</CardContent></Card>
  </>;
}
