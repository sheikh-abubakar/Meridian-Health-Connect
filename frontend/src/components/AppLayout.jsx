...
import {
  ...
  SidebarUserCard,
} from "@/components/MeridianSidebarBrand";
...
export function AppLayout() {
      ...
      .then((data) => {
        ...
        const paths = {
          ...
          frontdesk: [
            ...
            `${root}/appointments`,
            `${root}/doctors`,
            `${root}/recall-requests?status=pending_scheduling`,
          ],
          doctor: [
            `${root}/availability/${session.user.id}`,
            `${root}/appointments?status=checked_in`,
            `${root}/my-patients`,
  ...
  const navigation = [
    ...
    [
      ...
      `${root}/my-care-plans`,
      HeartHandshake,
      ["doctor", "care_coordinator"].includes(session.user.role),
    ],
    [
      "My Tasks",
      "Care coordination",
  ...
  const sidebar = (
    <>
      ...
      <nav className="sidebar-scrollbar flex-1 overflow-y-auto px-3 py-4">
        {session.user.role === "admin" && (
          <NavLink
            to={`/${tenantSlug}/overview`}
            className="mb-5 flex items-center gap-3 rounded-lg border border-slate-800 bg-slate-900 px-3 py-2.5 text-xs font-medium text-slate-300"
          >
            <span className="grid size-7 place-items-center rounded-md bg-slate-800 text-teal-300">
              ...
              {navigation
                ...
                .map(([label, , to, icon]) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      cn(
                        "group flex items-center gap-3 rounded-lg border border-transparent px-2.5 py-2 text-sm font-medium text-slate-300 transition hover:bg-slate-900 hover:text-white",
                        isActive &&
                          "border-slate-700 bg-slate-800 text-white shadow-[inset_3px_0_0_#2dd4bf]",
                      )
                    }
                  >
                    {({ isActive }) => (
                      <>
                        <span
                          className={cn(
                            "grid size-8 place-items-center rounded-md bg-slate-900 text-slate-400 group-hover:text-teal-300",
                            isActive && "bg-slate-700 text-teal-300",
                          )}
                        >
  ...
  return (
    <div className="min-h-screen min-w-0 overflow-x-hidden bg-slate-50 text-slate-950">
      <div
        className="fixed bottom-3 right-3 z-20 flex items-center gap-1.5 rounded-full border bg-white/90 px-2.5 py-1 text-[10px] text-muted-foreground shadow-sm backdrop-blur"
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-slate-800 bg-slate-950 text-white md:flex">
        {sidebar}
      </aside>
      <button
        type="button"
        ...
        className={cn(
          "fixed inset-0 z-40 bg-slate-950/55 backdrop-blur-[2px] transition-opacity duration-300 md:hidden",
          mobileOpen
      ...
      <aside
        ...
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-[min(82vw,18rem)] flex-col border-r border-slate-800 bg-slate-950 text-white shadow-2xl transition-transform duration-300 ease-out md:hidden",
      ...
      <div className="min-w-0 md:pl-64">
        <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-2 border-b bg-white/95 px-3 py-2 backdrop-blur md:px-8">
          <div className="flex min-w-0 items-center gap-2">
            <button
              ...
              className="relative grid size-10 shrink-0 place-items-center rounded-lg border bg-white text-slate-700 shadow-sm md:hidden"
            >
              <span
                className={cn(
                  "absolute h-0.5 w-5 rounded bg-current transition duration-300",
                  mobileOpen ? "rotate-45" : "-translate-y-1.5",
                )}
              />
              <span
                className={cn(
                  "absolute h-0.5 w-5 rounded bg-current transition duration-200",
                  mobileOpen ? "scale-x-0 opacity-0" : "scale-x-100",
                )}
              />
              <span
                className={cn(
            ...
            <div className="min-w-0">
              <p className="flex min-w-0 items-center gap-1.5 text-sm font-semibold">
                ...
                <span className="hidden shrink-0 items-center gap-1 text-teal-700 sm:flex">
                  <MapPin className="size-3.5" />
                  {location?.name || "Loading branch…"}
                </span>
              </p>
          ...
          <div className="flex shrink-0 gap-1.5">
            ...
            >
              <UserCircle className="size-4 sm:mr-2" />
        ...
        <main className="min-w-0 p-4 sm:p-5 md:p-8">
          <div className="mx-auto min-w-0 max-w-6xl">
            {escalationNotice && (
              <div className="mb-5 flex items-start justify-between gap-4 rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
                <span>
                  <strong>Task escalation:</strong> {escalationNotice}
                </span>
                <button
                  type="button"
