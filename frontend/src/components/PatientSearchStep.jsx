...
export function PatientSearchStep({ basePath, headers, onSelect, onCreate, allowCreate = true, excludePatientId = "" }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setResults(data.patients.filter((patient) => String(patient.id || patient._id) !== String(excludePatientId)).map((patient) => ({ ...patient, id: patient.id || patient._id })));
      } catch (requestError) { setError(requestError.message); }
      finally { setSearching(false); }
    }, 300);
    return () => clearTimeout(timer);
  }, [basePath, excludePatientId, headers, query]);
  ...
  return <div className="space-y-4">
    ...
    {allowCreate && <div className="border-t pt-4"><Button type="button" variant="outline" className="w-full" onClick={() => onCreate(query)}><UserRoundPlus className="mr-2 size-4" /> Add a new patient anyway</Button></div>}
  </div>;
}
